...
import numpy as np
import optax
from jax import random, jit, lax, grad
...
from info_SSAeqn import vectgrad, gov_eqn, front_eqn
from data_load import icesheet_data
...
if __name__ == "__main__":
    ...
    outdir = rootdir.joinpath('SynData')
    ...
    NN_loss = loss_create(pred_u, scale, lw, loss_ref=loss0)
    ...
    for l in range(3):
        ...
        mdic = {"x": np.asarray(x), "y": np.asarray(y), "u": np.asarray(u_p), "v": np.asarray(v_p), "h": np.asarray(h_p),
                ...
                "mu": np.asarray(mu_p), "params": pm, "loss": np.asarray(loss_all),
                "v_mean": np.asarray(dmean), "v_range": np.asarray(drange), "mu0": np.asarray(mu0)}
        FileName = 'SynData_SGD_idx%3d_iter%1d.mat' % (seed, l)
